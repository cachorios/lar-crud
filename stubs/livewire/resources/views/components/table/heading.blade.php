...
@php
    $direction = null;
    if (isset($sorted[$sortField]))
        $direction = $sorted[$sortField] ? $sorted[$sortField] : null ;
@endphp
...
>
    ...
    @else
        <button wire:click="sortBy('{{ $sortField }}')" {{ $attributes->except('class') }} class="flex items-center space-x-1 text-left text-xs leading-4 font-medium text-cool-gray-500 uppercase tracking-wider group focus:outline-none focus:underline">
            <span>{{ $slot }}</span>
